...
The Pterodactyl (7) & Dragon (8) "Leg" TrapYour classifyLimbs script checks if a foot lands within 72% of the mean foot depth. If the Pterodactyl walks on its wing-wrists, the system will incorrectly flag them as legs, breaking the walking physics and animation states.The Fix: Add a specific Metadata Tag or Bone Prefix (e.g., WNG_L_Wrist) to the skeleton template. Override classifyLimbs to explicitly ignore bones with the WNG prefix for depth classification, treating them as custom locomotion anchors instead.The Serpent (4) Physics ConstraintsA pure legless serpent on a standard physics controller often experiences "snaking" clipping issues or gets stuck on terrain meshes.The Fix: Ensure its locomotion uses a spline-follower or custom capsule-chain collider rather than a single standard capsule component, preventing the head or tail from clipping through geometry during tight turns.2. Silhouette & Gameplay RefinementsformattingDirect Comparisons: Key Distinctions between Pterodactyl and Dragon
Use code with caution.Feature7. Pterodactyl8. DragonMass DistributionWing-dominant, tiny rear legsEven quadruped, 4 equal heavy limbsGround LocomotionAwkward, relies on wing-knucklesStable, walks easily as a true quadrupedWing EdgesSmooth, translucent, single finger spar4 sharp projecting finger-spar claw tipsTail FunctionNegligible / StabilizingHeavy, thick base, used actively for balanceThe Stalker (6) vs. Husked Alien (9)Both use a "hard digitigrade back-rake" for speed. To ensure they do not visually blur during fast-paced gameplay:Scale and Height Profile: Keep the Stalker (6) low to the ground with a horizontal, rodent-like posture. Keep the Husked Alien (9) vertically elongated, letting it unfold its extra-jointed limbs to tower over the player during agro states.Armadillo-Lizard (10) Systems IntegrationYou mentioned wanting its high health and low speed to fall out naturally from the mass-driven attribute system.The Adjustment: Ensure your systemic formula scales linearly for mass vs. health, but scales exponentially for armor plating. True interlocking rock plates should calculate directional damage reduction (e.g., +80% defense on the back, but standard defense on the soft exposed flesh seams underneath) to reward tactical player positioning.
