...
- **Zone: irregular, off-centre, lands anywhere** (evening). Seeded three-lobe wobble on the edge, a drifting centre and a seeded final ring on dry land. The close rate had to go from 11 to 22 units a second and the opening hold from 150s to 75s, or the wall never reached anyone — see the changelog.
- **Flight: fixed and rebuilt** (evening). Altitude no longer rides the terrain, and a flap is one key press whose lift scales hard with wing area. Still to do from the roadmap's flight section: thermals are in but stall recovery, banking feel and landing flare want a pass.
